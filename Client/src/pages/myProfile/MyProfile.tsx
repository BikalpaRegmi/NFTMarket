    ...
    try {
      ...
      const newRes: MyNftsType[] = await Promise.all(
        res.map(async (curval: any) => {
          ...
          if (curval.owner.toLowerCase() === account?.toLowerCase() && curval.isListed == false) {
            return {
              name: metaData.name,
          ...
          // aba ayexi check out bana ani sabai nft haru display hune bana ani na huda no nft to show pani bana ani finished problem chai k vayo vanda time sakepaxi ayera my nft m save hunxa ki hudaina vanera chek garnai payena k mula ni relist pani banauna hunthyo
        })
      );
      ...
      <section className="py-16">
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            {myNft.length>0 ? myNft.map((curval: MyNftsType) => {
              return (
            ...
            }) : <p className="text-xl mx-auto text-blue-500 animate-pulse w-full">You Dont Own Any Nft That Is Not Listed</p>}
          </div>
        </div>
      </section>
    </div>
